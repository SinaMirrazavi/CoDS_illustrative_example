...
if abs(p(1)*X_free(1)+p(2)-X_free(2))<0.1
    t = text(limits(1,1),(limits(1,4)+limits(1,3))/2,{'ERROR: the chosen point is very close to the cotact surface.';'Please pick one point which is far from the contact surface.'});
    s = t.FontSize;
    t.FontSize = 20;
    X_free= ginput(1)';
    if abs(p(1)*X_free(1)+p(2)-X_free(2))<0.1
        close all
        clc
        t = text(limits(1,1),(limits(1,4)+limits(1,3))/2,{'ERROR: the chosen point is very close to the cotact surface.';'Please start again and pick one point which is far from the contact surface.'});
        s = t.FontSize;
        t.FontSize = 20;
        pause(1)
        error('ERROR: the chosen point is very close to the cotact surface. Please start again and pick one point which is far from the contact surface.')
    end
end
% X_free=[0 ;0];
...
createfigure_with_wall(X(1,:),X(2,:),limits,'Specify the target position by adding one data point on the contact surface.');
X_Contact= ginput(1)';
